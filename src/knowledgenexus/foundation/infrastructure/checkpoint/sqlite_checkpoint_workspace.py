...
_DDL = (
    ...
    "CREATE TABLE raw_page_progress (run_id TEXT NOT NULL, generation_id TEXT NOT NULL, page_id TEXT NOT NULL CHECK (length(page_id) > 0), request_profile_version TEXT NOT NULL CHECK (request_profile_version = 'm7-confluence-request-profile-v1'), source_version TEXT, http_status INTEGER NOT NULL CHECK (http_status >= 100 AND http_status <= 599), raw_sha256 TEXT NOT NULL CHECK (length(raw_sha256) = 64), byte_count INTEGER NOT NULL CHECK (byte_count >= 0), status TEXT NOT NULL CHECK (status IN ('committed')), committed_at TEXT NOT NULL CHECK (length(committed_at) = 24 AND COALESCE(strftime('%Y-%m-%dT%H:%M:%fZ', committed_at) = committed_at, 0)), PRIMARY KEY (run_id, page_id), FOREIGN KEY (run_id) REFERENCES crawl_runs(run_id))",
    "CREATE TABLE raw_restriction_progress (run_id TEXT NOT NULL, generation_id TEXT NOT NULL, selected_page_id TEXT NOT NULL CHECK (length(selected_page_id) > 0), target_page_id TEXT NOT NULL CHECK (length(target_page_id) > 0), request_profile_version TEXT NOT NULL CHECK (request_profile_version = 'm7-confluence-request-profile-v1'), http_status INTEGER NOT NULL CHECK (http_status IN (200,401,403,404)), raw_sha256 TEXT NOT NULL CHECK (length(raw_sha256) = 64), byte_count INTEGER NOT NULL CHECK (byte_count >= 0), status TEXT NOT NULL CHECK (status IN ('committed')), committed_at TEXT NOT NULL CHECK (length(committed_at) = 24 AND COALESCE(strftime('%Y-%m-%dT%H:%M:%fZ', committed_at) = committed_at, 0)), PRIMARY KEY (run_id, selected_page_id, target_page_id), FOREIGN KEY (run_id) REFERENCES crawl_runs(run_id))",
...
_OBJECTS = {"checkpoint_metadata", "crawl_runs", "crawl_sessions", "include_roots", "root_occurrences", "root_progress", "inventory_windows", "inventory_occurrences", "raw_page_progress", "raw_restriction_progress", "checkpoint_transitions", "request_budget_reservations", "idx_crawl_sessions_run_started", "idx_crawl_sessions_one_active", "idx_inventory_occurrences_run_page", "idx_inventory_windows_run_root", "idx_root_progress_incomplete"}
...
_TABLE_LAYOUTS = {
    ...
    "raw_page_progress": (("run_id", "TEXT", 1, 1), ("generation_id", "TEXT", 1, 0), ("page_id", "TEXT", 1, 2), ("request_profile_version", "TEXT", 1, 0), ("source_version", "TEXT", 0, 0), ("http_status", "INTEGER", 1, 0), ("raw_sha256", "TEXT", 1, 0), ("byte_count", "INTEGER", 1, 0), ("status", "TEXT", 1, 0), ("committed_at", "TEXT", 1, 0)),
    "raw_restriction_progress": (("run_id", "TEXT", 1, 1), ("generation_id", "TEXT", 1, 0), ("selected_page_id", "TEXT", 1, 2), ("target_page_id", "TEXT", 1, 3), ("request_profile_version", "TEXT", 1, 0), ("http_status", "INTEGER", 1, 0), ("raw_sha256", "TEXT", 1, 0), ("byte_count", "INTEGER", 1, 0), ("status", "TEXT", 1, 0), ("committed_at", "TEXT", 1, 0)),
...
_FOREIGN_KEY_LAYOUTS = {
    ...
    "raw_page_progress": ((0, 0, "crawl_runs", "run_id", "run_id", "NO ACTION", "NO ACTION", "NONE"),),
    "raw_restriction_progress": ((0, 0, "crawl_runs", "run_id", "run_id", "NO ACTION", "NO ACTION", "NONE"),),
...
_INDEX_LAYOUTS = {
    ...
    "raw_page_progress": ((0, "sqlite_autoindex_raw_page_progress_1", 1, "pk", 0, ((0, 0, "run_id", 0, "BINARY", 1), (1, 2, "page_id", 0, "BINARY", 1), (2, -1, None, 0, "BINARY", 0))),),
    "raw_restriction_progress": ((0, "sqlite_autoindex_raw_restriction_progress_1", 1, "pk", 0, ((0, 0, "run_id", 0, "BINARY", 1), (1, 2, "selected_page_id", 0, "BINARY", 1), (2, 3, "target_page_id", 0, "BINARY", 1), (3, -1, None, 0, "BINARY", 0))),),
    "crawl_sessions": (
